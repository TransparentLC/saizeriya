...
const menu = JSON.parse(fs.readFileSync('menu.json', {encoding: 'utf-8'}));
...
for await (const _ of asyncPool(16, menu, async e => {
    ...
    fs.writeFileSync(`../../public/image/${basename}.jpg`, imgData);
    fs.writeFileSync(`../../public/image/${basename}.webp`, await imgResized.webp({
        quality: 75,
        effort: 6,
        smartSubsample: true,
    }).toBuffer());
    fs.writeFileSync(`../../public/image/${basename}.avif`, await imgResized.avif({
        quality: 50,
        effort: 9,
